...
case class CheckedDef(params:List[(String,Int)], body:Expr)

// shameful imperative code...

object Linearize {
  
  def apply(prog:Map[String,CheckedDef]):Array[Byte] = new Linearizer(prog).run
  
}

class Linearizer(prog:Map[String,CheckedDef]) {
